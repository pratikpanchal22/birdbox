...
class AppSettings:
    ...
    def isMotionTriggerActive(self):
        ...
        dv = True
        try:
            dv = self._settingsJson[dbc.KEY_MOTION_TRIGGERS][dbc.KEY_ENABLED] 
        except:
            logger("_ERROR_", "appSettings[0][dbc.KEY_MOTION_TRIGGERS][dbc.KEY_ENABLED] doesn't exist")
        return dv

    def isSilentPeriodActive(self):
        dv = False
        try:
            silentPeriodEnabled = self._settingsJson[dbc.KEY_SILENT_PERIOD][dbc.KEY_ENABLED]
            silentStartTime = parser.parse(self._settingsJson[dbc.KEY_SILENT_PERIOD][dbc.KEY_START_TIME])
            silentEndTime = parser.parse(self._settingsJson[dbc.KEY_SILENT_PERIOD][dbc.KEY_END_TIME])
        except:
    ...
    def maxNumberOfAllowedSimultaneousChannels(self):
        ...
        try:
            ...
            logger("_ERROR_", "appSettings[0][dbc.KEY_SETTINGS][dbc.KEY_SYMPHONY][dbc.KEY_MAXIMUM] doesn't exist")
        return dv
    ...
    def isBirdChoiceLimitedToSameSpecies(self):
        ...
        dv = False
        try:
            dv = self._settingsJson[dbc.KEY_SYMPHONY][dbc.KEY_LIMIT_TO_SAME_SPECIES] 
        except:
            logger("_ERROR_", "appSettings[0][dbc.KEY_SETTINGS][dbc.KEY_SYMPHONY][dbc.KEY_LIMIT_TO_SAME_SPECIES] doesn't exist")
        return dv
    ...
    def maxVolume(self):
        ...
        dv = 100
        try:
            dv = int(self._settingsJson[dbc.KEY_VOLUME])
        except:
            logger("_ERROR_", "appSettings[0][dbc.KEY_SETTINGS][dbc.KEY_VOLUME] doesn't exist")
        return dv
